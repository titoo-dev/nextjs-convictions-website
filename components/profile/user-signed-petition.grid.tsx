...
export async function UserSignedPetitionsGrid() {
	...
	const signedPetitions = await getUserSignedPetitions(locale);
	...
	if (!signedPetitions || signedPetitions.length === 0) {
		return (
			<div className="text-center py-12">
				...
				<Button className="mt-4">{t('browse-petitions')}</Button>
			</div>
		);
	}
	...
	return (
		<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
			{signedPetitions.map((petition) => (
				<UserSignedPetitionCard key={petition.id} petition={petition} />
			))}
		</div>
	);
}
